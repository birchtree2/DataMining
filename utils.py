...
def get_repre_sequences(distance_matrix, k, random_state=None):
    kmedoids = KMedoids(n_clusters=k, metric='precomputed', init='k-medoids++', random_state=random_state)
...
def get_metrics(predict_distance_matrix, target_distance_matrix, topks=[1, 5, 10]):
    ...
    metrics = {}
    for topk in topks:
        metrics['top%d' % topk] = round(topk_accuracy(predict_labels, target_labels, topk),3)
    return metrics
